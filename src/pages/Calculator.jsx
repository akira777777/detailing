      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-12">
            ...
            <section>
              <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">01. Select Vehicle Geometry</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {['sedan', 'suv', 'sport', 'luxury'].map((type) => (
                    <label key={type} className="group relative cursor-pointer">
                        ...
                        />
                        <div className="bg-panel-dark border border-white/10 p-6 flex flex-col items-center gap-4 transition-all duration-300 peer-checked:border-primary peer-checked:bg-primary/5 group-hover:border-white/30 rounded-xl">
                            <span className={`material-symbols-outlined text-4xl transition-colors ${vehicle === type ? 'text-primary' : 'text-white/20 group-hover:text-white'}`}>
                                {type === 'sedan' ? 'directions_car' : type === 'suv' ? 'airport_shuttle' : type === 'sport' ? 'speed' : 'diamond'}
                            </span>
                            <span className="text-[11px] font-black uppercase tracking-[0.1em] text-white">{type}</span>
            ...
            <section>
              <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">02. Paint Surface Assessment</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    <label key={item.id} className="cursor-pointer">
                        ...
                        />
                        <div className="bg-panel-dark border border-white/5 p-8 transition-all duration-300 peer-checked:border-primary/40 peer-checked:bg-primary/5 rounded-xl h-full flex flex-col justify-between">
                            <div>
                                <h4 className="text-lg font-black uppercase tracking-tight mb-2 text-white">{item.label}</h4>
                                <p className="text-white/30 text-[11px] leading-relaxed mb-6">{item.desc}</p>
                            </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">03. Core Service Modules</h2>
                 ...
                 {[
                    ...
                    <div key={mod.id} className={`bg-panel-dark border p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all rounded-xl ${modules[mod.id] ? 'border-primary/30 bg-primary/5' : 'border-white/5 hover:border-white/20'}`}>
                        <div className="flex gap-6 items-start md:items-center">
                            <div className="size-12 bg-white/5 border border-white/10 flex items-center justify-center shrink-0 rounded-lg">
                                <span className={`material-symbols-outlined ${modules[mod.id] ? 'text-primary' : 'text-white'}`}>{mod.icon}</span>
                            </div>
                            <div>
                                <h4 className="text-lg font-black uppercase tracking-tight text-white">{mod.title}</h4>
                                <p className="text-white/40 text-[11px] font-medium tracking-wide">{mod.desc}</p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between md:justify-end gap-8">
                            <div className="text-right">
                                <span className="text-white/40 text-[10px] uppercase font-bold tracking-widest block">Est. Cost</span>
                                <span className="text-xl font-black text-white">${mod.cost}</span>
                            </div>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-28">
            <div className="glass-panel p-8 border border-white/10 relative overflow-hidden rounded-xl">
