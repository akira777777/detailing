...
const Booking = () => {
        ...
        try {
            const response = await fetch('/api/booking', {
                ...
                body: JSON.stringify({
                    ...
                    time: selectedTime,
                    carModel: carModel,
                    packageName: getPackageName(modules, t),
    ...
    const getTimeLabel = (label) => {
        ...
        if (label === 'Fully Booked') return t('booking.fully_booked');
        return label;
    };
                            ...
                            <div className="grid grid-cols-7 gap-2">
                                ...
                                {days.map(({ day, fullDate, isWeekend }) => {
                                    ...
                                    return (
                                        <button
                                            key={day}
                                            onClick={() => setSelectedFullDate(new Date(viewDate.getFullYear(), viewDate.getMonth(), day))}
                                            aria-label={`Select ${fullDate}${isWeekend ? ' - Weekend Special' : ''}`}
                                            aria-pressed={isSel}
                                            className={`h-14 flex flex-col items-center justify-center rounded-lg text-sm font-semibold transition-all relative ${isSel ? 'bg-primary text-white shadow-lg shadow-primary/30 transform scale-105' : 'hover:bg-gray-100 dark:hover:bg-white/10 text-gray-900 dark:text-white'}`}
                                        >
                                            <span>{day}</span>
                        ...
                        <div className="flex flex-col gap-4 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 p-5 shadow-xl">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <p className="text-primary text-sm font-bold">{getPackageName(modules, t)}</p>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    ...
                                    <button
                                        ...
                                        aria-label={`${slot.time} ${getTimeLabel(slot.label)}${!slot.avail ? ' - Fully Booked' : ''}`}
                                    ...
                                    >
                                        ...
                                        <span className={`text-[10px] uppercase font-bold ${!slot.avail ? 'text-gray-300 dark:text-white/20' : selectedTime === slot.time ? 'text-white/70' : 'text-gray-400 dark:text-white/40 group-hover:text-primary transition-colors'}`}>{getTimeLabel(slot.label)}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
