        ...
        <div className="pt-32 pb-24 px-6 lg:px-12 bg-background-dark min-h-screen">
             <div className="max-w-[1440px] mx-auto">
                <div className="flex flex-col gap-4 mb-16">
                    ...
                    <h1 className="text-white text-5xl font-black leading-tight tracking-[-0.033em] uppercase">Transformation<br/>Gallery</h1>
                ...
                <div className="flex flex-wrap gap-3 items-center mb-12">
                    ...
                    {['All', 'Luxury', 'SUV', 'Correction', 'Protection'].map(cat => (
                         <button
                            ...
                            className={`flex h-9 items-center justify-center gap-x-2 rounded-full border px-6 text-xs font-bold uppercase tracking-wider transition-all ${filter === cat ? 'bg-primary border-primary text-white' : 'border-white/10 text-white/60 hover:border-primary hover:text-white'}`}
                        >
                            {cat}
                        </button>
                    ))}
                ...
                <div className="grid grid-cols-1 gap-12">
                    {transformations.map((item) => (
                        <div key={item.id} className="group bg-panel-dark rounded-xl overflow-hidden border border-white/5 hover:border-primary/50 transition-all shadow-2xl">
                                ...
                                <div className="p-8 lg:p-12 flex flex-col justify-between">
                                    <div>
                                        <div className="flex items-center gap-2 mb-4">
                                            <span className="px-3 py-1 rounded bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider border border-primary/20">{item.category}</span>
                                        </div>
                                        ...
                                        <div className="space-y-4 mb-8">
                                            {item.services.map((service, i) => (
                                                <div key={i} className="flex items-start gap-3">
                                                    ...
                                                    <p className="text-white/60 text-sm leading-relaxed">{service}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between border-t border-white/5 pt-8">
                                        ...
                                        <button className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest hover:text-white transition-colors">
