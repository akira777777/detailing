                    ...
                    <nav className="flex flex-col gap-2">
                        {dashboardMenu.map((item, i) => (
                            <div key={item.name} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-colors ${i === 1 ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-gray-600 dark:text-white/60 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                                ...
                                <p className="text-sm font-semibold">{t(`dashboard.sidebar.menu.${item.name}`)}</p>
                            </div>
                        ))}
                    </nav>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="p-4 bg-primary/10 rounded-xl border border-primary/20">
                        <div className="flex justify-between items-center mb-2">
                            <p className="text-xs font-bold text-primary uppercase">{t('dashboard.sidebar.loyalty_points')}</p>
                        ...
                        <p className="text-[10px] text-gray-500 dark:text-white/60 mt-2 italic">{userData.nextReward - userData.loyaltyPoints} {t('dashboard.sidebar.pts_next_reward')}</p>
                    </div>
                    <Link
                        to="/booking"
                    ...
                    >
                        ...
                        {t('dashboard.sidebar.book_new_service')}
                    </Link>
                </div>
            </aside>
            ...
            <div className="flex-1 px-6 lg:px-10 py-8 lg:ml-72">
                ...
                <header className="flex flex-wrap justify-between items-end gap-4 mb-8">
                    <div className="flex flex-col gap-1">
                        <h2 className="text-gray-900 dark:text-white text-4xl font-black leading-tight tracking-tight">{t('dashboard.title')}</h2>
                        <p className="text-gray-600 dark:text-white/60 text-base font-normal">{t('dashboard.subtitle')}</p>
                    </div>
                ...
                <section className="mb-10">
                    <div className="flex items-center gap-2 mb-4 px-1">
                        ...
                        <h3 className="text-gray-900 dark:text-white text-xl font-bold tracking-tight">{t('dashboard.status.title')}</h3>
                    </div>
                        ...
                        <div className="flex-1 p-8 flex flex-col justify-between">
                            <div className="flex flex-col gap-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-primary text-xs font-bold uppercase tracking-widest mb-1">{t('dashboard.status.active_detailing')}</p>
                                        <p className="text-gray-900 dark:text-white text-2xl font-extrabold tracking-tight">{activeService.status}</p>
                                        <p className="text-gray-600 dark:text-white/60 text-base mt-2">{t('dashboard.status.vehicle')}: <span className="text-gray-900 dark:text-white font-medium">{activeService.vehicle}</span></p>
                                    </div>
                                    <button
                                        onClick={() => addToast(t('dashboard.status.connecting_feed'), 'warning')}
                                    ...
                                    >
                                        ...
                                        {t('dashboard.status.live_camera')}
                                    </button>
                                </div>
                            ...
                            <div className="mt-6 flex items-center gap-4 text-sm text-gray-600 dark:text-white/60">
                                <div className="flex items-center gap-2">
                                    ...
                                    {t('dashboard.status.estimated_completion')}: <span className="text-gray-900 dark:text-white">{activeService.completion}</span>
                                </div>
                                <div className="h-1 w-1 bg-gray-400 dark:bg-white/20 rounded-full"></div>
                                <div className="flex items-center gap-2">
                                    <span className="material-symbols-outlined text-sm">person</span>
                                    {t('dashboard.status.master_detailer')}: <span className="text-gray-900 dark:text-white">{activeService.master}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    <div className="flex justify-between items-center mb-6 px-1">
                        <h3 className="text-gray-900 dark:text-white text-xl font-bold tracking-tight">{t('dashboard.history.title')}</h3>
                        <div className="flex gap-2">
                            <div className="relative group">
                                ...
                                <input className="bg-white dark:bg-panel-dark border-gray-200 dark:border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-gray-900 dark:text-white focus:ring-primary focus:border-primary w-64 border outline-none shadow-sm dark:shadow-none" placeholder={t('dashboard.history.search_placeholder')} type="text" aria-label="Search service history" />
                            </div>
                            <button
                                aria-label={t('dashboard.history.filter_aria')}
                    ...
                    <div className="bg-white dark:bg-panel-dark rounded-xl border border-gray-200 dark:border-white/5 overflow-hidden transition-colors shadow-sm dark:shadow-none">
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-white/60 text-xs font-bold uppercase tracking-wider">
                                        <th className="px-6 py-4">{t('dashboard.history.columns.date')}</th>
                                        <th className="px-6 py-4">{t('dashboard.history.columns.service')}</th>
                                        <th className="px-6 py-4">{t('dashboard.history.columns.vehicle')}</th>
                                        <th className="px-6 py-4">{t('dashboard.history.columns.cost')}</th>
                                        <th className="px-6 py-4">{t('dashboard.history.columns.status')}</th>
                                        <th className="px-6 py-4 text-right">{t('dashboard.history.columns.invoice')}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                    {isLoading ? (
                                        ...
                                        <tr>
                                            <td colSpan="6" className="px-6 py-10 text-center text-gray-500 dark:text-white/40">
                                                {t('dashboard.history.no_history')}
                                            </td>
                                        </tr>
                                    ) : (
                                            ...
                                            <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                                ...
                                                <td className="px-6 py-5">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-gray-500 dark:text-white/60 text-xs">{booking.time} {t('dashboard.appointment')}</span>
                                                    </div>
                                                </td>
                        ...
                        <div className="px-6 py-4 bg-gray-50 dark:bg-white/5 flex justify-between items-center text-gray-500 dark:text-white/60 text-xs border-t border-gray-100 dark:border-white/5">
                            <p>{t('dashboard.history.showing', { start: Math.min((currentPage - 1) * pageSize + 1, totalBookings), end: Math.min(currentPage * pageSize, totalBookings), total: totalBookings })}</p>
                            <div className="flex gap-2">
                                <button
